...
| 2026-06-12 | Claude (claude-sonnet-4-6) | `claude/keen-noether-23jwsg` | **Platform v2 설계 확정** (gstack office-hours, 3라운드 리뷰 9/10): 앱=계정/포스트=콘텐츠 모델, 저장≠팔로우, UGC 후기 허용, 콜드스타트 큐레이션+클레임. 설계 문서: `~/.gstack/projects/ordinary-first-appflow/moon-claude-keen-noether-23jwsg-design-20260612-090914.md`. **Phase 1 스키마 마이그레이션 완료**: `drizzle/0001_platform_v2.sql` — posts/likes/follows/comments/saves/notifications 테이블 + 인덱스, `glim-system` 시스템 유저 seed, apps에 icon_url/follower_count/save_count, user에 last_notif_check_at, status enum에 'unclaimed' 추가. 기존 apps→official post 백필, interactions(save/like)→saves/likes 이관. seed.sql에 posts 백필 추가. 로컬 D1 적용·검증 + typecheck 통과. **주의: 원격 D1에는 아직 미적용** (`pnpm db:migrate:remote` 필요). |
| 2026-06-12 | Claude (claude-sonnet-4-6) | `claude/keen-noether-23jwsg` | **Platform v2 Phase 2~6 전체 구현 완료.** Phase 2: 피드를 posts 모델로 전환(`getFeedPosts`), AppCard 이미지 슬라이드쇼(스냅 스와이프+탭존+진행바, Chromium snap-mandatory smooth-scroll 버그 우회), Submit 폼 Video/Screenshots 모드(1–5장), POST /api/apps가 official post 동시 생성. Phase 3: BottomNav 5탭(Home/Saved/+/Inbox/Profile), 피드 상단 For You/Following/Categories 탭+카테고리 칩, /notifications·/profile 페이지. Phase 4: /api/follows 토글(app|user, followerCount 캐시 동기화), /api/feed?type=following(팔로우 앱 모든 포스트 ∪ 팔로우 유저 작성 포스트), AppCard 아바타+팔로우 배지, 낙관적 토글. Phase 5: /api/comments(대댓글 1단계 강제, commentCount 캐시, 작성자 알림), CommentsSheet 바텀시트, 알림 페이지 실데이터+전체읽음, BottomNav 미읽음 배지, 팔로우 알림. Phase 6: `drizzle/0002_claim_requests.sql`, POST /api/claims(미등록 앱 클레임 신청), POST /api/claims/approve(x-admin-token 가드, makerId/포스트 이관+알림), /app/[slug]에 unclaimed 배지+ClaimButton+FollowAppButton+포스트 탭(All/Official/Reviews), /maker/[userId] 공개 프로필(앱 그리드+리뷰 목록). 전체 typecheck 통과, 로컬 D1 마이그레이션 적용, 프리뷰 브라우저 기능 검증 완료. |
| 2026-06-12 | Claude (claude-sonnet-4-6) | `claude/keen-noether-23jwsg` | **Platform v2 프로덕션 배포 완료.** 원격 D1에 0001+0002 마이그레이션 적용 (12 apps → 12 official posts 백필, glim-system 유저 seed 확인). `CLAIM_ADMIN_TOKEN` 시크릿 생성·주입 (값은 로컬 `.dev.vars`에 보관, gitignored). WSL에서 OpenNext 빌드·배포 (Version 1fb88fef). 프로덕션 검증: 전체 페이지 200 (/, /saved, /notifications, /profile, /maker/seed-maker, /app/breathbox), 새 네비게이션(For You/Following/Categories + 하단 5탭) HTML 확인, API 가드 정상(익명 follow 401, 무토큰 approve 403). 참고: Windows wrangler는 미인증 — 원격 D1/배포 작업은 WSL(`~/glim`)에서 실행할 것. |
| 2026-06-12 | Claude (claude-sonnet-4-6) + Codex (gpt-5.5) | `claude/keen-noether-23jwsg` | **카탈로그 30개 확장 + 앱/maker 페이지 개선 + 프로덕션 배포.** **앱 상세 페이지 재구성** (`src/app/app/[slug]/page.tsx`): 결정 페이지 UX (트러스트 시그널 → 미디어 → Try → 접힘 설명 → top reviews → made by), `DescriptionExpander`로 긴 설명 접기, 사람들의 반응 인라인. **Made by 클릭 활성화** (#16): unclaimed 앱은 maker 이름이 외부 X/website/github 링크로(외부 표시 아이콘 포함), claimed는 `/maker/{id}`. **AppCard 영상 버그 픽스** (`src/components/feed/AppCard.tsx`): `isSlideshow` 우선 → `demoVideoUrl → youtubeUrl → isSlideshow` 순으로 변경. curated 앱이 이제 피드에서 영상 재생. **maker 프로필 페이지 개선** (#17): 큰 아바타, 𝕏/website/GitHub 소셜 링크 행, 리뷰가 quote 카드 형태(🖼 폴백 제거). **신규 22개 큐레이션** (Codex) → 총 30개. `seed-curated-v2.sql` (Plinq, Outlit, fly.pieter.com, BetriqAI, Tastly, PathCraft, WanderScout, Norte Wallet, Ready Steady Find, Wise Compass, MindSpark Study, Brand Studio, ResumeAgent, CutList Optimizer, GoldMine AI, NinjaAlert, TrackingPass, SVG Doodle, Vibe Coders Globe, Great Taxi Assignment, Vibeware, Vector Tango), 외부 og:image 사용, fly.pieter.com에 YouTube 추가. `seed-curated-video-backfill.sql` 기존 8개 앱 중 6개 YouTube URL 추가 (Lovelee, Vugola, FULL SEND, 82-0, BirdsEyes, Dreambase) + 풍부한 image_urls/image_captions. **데모 시드** (`seed-demo.sql`, LOCAL ONLY, gitignored 안됨—주의): BirdsEyes를 demo-user-christian에 claim, 12개 리뷰 + 10개 댓글 + try_click/feedback 트러스트 시그널. **Codex CLI 버그 픽스**: `~/.codex/config.toml`의 `service_tier = "priority"`가 CLI 파싱 실패 원인 (CLI는 fast/flex만 지원) — 라인 제거. **프로덕션 배포**: WSL `~/glim`에서 `pnpm run deploy` (Version 806ed637), 원격 D1에 v2 카탈로그 + 백필 적용 (242 + 12 rows written). 검증: `/`, `/app/full-send`, `/app/plinq` 모두 200, FULL SEND에서 Daniel Vassallo → x.com/dvassallo 링크 확인. |
...
| 2026-06-13 | Claude (claude-fable-5) | `claude/keen-noether-23jwsg` | **Platform 1b+1c+Part 2 (커밋 f5fbc71, 1770aa1, 2fd5bad).** 1b: 비디오 윈도잉(active±1만 `<video>`), Try CTA 단일소스(`lib/try-target.ts`+`TryLink` — non-embeddable 웹앱이 /try 함정에 안 빠지고 외부 오픈도 try_click 기록), CommentsSheet 에러표시+IME. 1c: 알림 페이지 inArray 수리+배지(미읽음+팔로우 새포스트 join), Dialog 포커스트랩+스크롤락, rail aria-label+44px 터치타깃, URL 전수 프로토콜 검증, admin 토큰 상수시간, 앱삭제 follows 정리, 업로드 25MB, seed.sql→seed-dev-dummy.sql(+db:seed:remote 제거), 루트 PNG/세션문서 아카이브(`~/.gstack/projects/ordinary-first-appflow/archive/`). **Part 2 성장루프 6종**: E1 `/search`+`/api/search`(LIKE 이스케이프) E3 피드백→리뷰 원탭(긍정+20자+로그인, mediaType='text' 신설—비디오 피드 제외, 본인삭제 `DELETE /api/posts/[id]`) E4 클레임랜딩 "N명 도달"+utm_source(0007) E5 `/api/badge/[slug]` SVG(XML이스케이프, 1h캐시, badge_view 로깅)+대시보드 임베드복사 E6a `/trending`(7일 dedupe 단일쿼리+15분 캐시) E2 대시보드 2.0(`app_daily_stats` 0008, lazy 풀데이 upsert, 14일 추이바+전주대비, merge 시 롤업 무효화). **참고**: text 리뷰는 팔로잉 피드에서도 제외(텍스트 카드 디자인 부재—디자인 리뷰 후 TODOS), 다음 권장: 라이브에서 `/gstack-design-review`. 플랜: `docs/designs/platform-stabilization-growth.md`. |
